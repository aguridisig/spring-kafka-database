...
class UserKafkaProducerTest
{
    ...
    void setUp() {
        ...
        ContainerProperties containerProperties = new ContainerProperties("com.example.kafka.user");
    ...
    @Test
    void testWriteToKafka() throws JsonProcessingException, InterruptedException
    {
        // Create a user and write to Kafka
        final UserDTO userDto = new UserDTO("11111", "John", "Wick");
        producer.writeToKafka(userDto);

        // Read the message (John Wick user) with a test consumer from Kafka and assert its properties
        final ConsumerRecord<String, String> message = records.poll(500, TimeUnit.MILLISECONDS);
        assertNotNull(message);
        assertEquals("11111", message.key());
        final UserDTO result = objectMapper.readValue(message.value(), UserDTO.class);
